/* Malex icon set — derived from the lllX. 24px grid, round join/cap.
   ...
   Usage: malexIconSVG('locker', { size: 24, stroke: 2, color: 'var(--navy-900)' }) → inner SVG markup string. */
export const MALEX_ICONS = {
...
const ALIASES = { "map-pin": "location", "shield-check": "shield", "credit-card": "card", "x": "close", "log-in": "arrow-right", "chevron": "chevron-right" };
for (const k in ALIASES) { if (MALEX_ICONS[ALIASES[k]]) MALEX_ICONS[k] = MALEX_ICONS[ALIASES[k]]; }

export function malexIconInner(name) {
  return MALEX_ICONS[name] || null;
}
