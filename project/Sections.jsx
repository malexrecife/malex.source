    ...
    <section className="sec-navy on-navy" id="precos">
      ...
      <div className="container precos-inner">
              ...
              <div className={`price-card${hot ? " price-card-hot" : ""}`} key={k}>
                ...
                <div className="price-card-big">
                  <span className="price tabular">
                    <span className="price-cur">R$</span>{p.h4}
                  </span>
                  <span className="price-unit">/ até 4h</span>
                </div>
                <div className="price-card-day tabular">R$ {p.day} / diária</div>
                <Btn
        ...
        <p className="precos-note t-body-sm">
          Valores ilustrativos por unidade · tarifa até 4h ou diária cheia.
        </p>
      </div>
    </section>
  );
          ...
          {Object.entries(cols).map(([h, items]) => (
            <div key={h}>
              ...
              <ul className="footer-list">
                {items.map((it) => (
                  <li key={it.label}>
                    <a href={`#${it.id}`} onClick={(e) => { e.preventDefault(); window.scrollToId(it.id); }}>{it.label}</a>
                  </li>
                ))}
              </ul>
            </div>
...
Object.assign(window, { Pricing, Sobre, Contato, Footer, PROOF });
